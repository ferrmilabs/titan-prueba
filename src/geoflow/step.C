...
//#endif
	...
	while(currentPtr)
	  {
	    Element* Curr_El=(Element*)(currentPtr->value);
 	    if(Curr_El->get_adapted_flag()>0) { //if this is a refined element don't involve!!!
	      ...
	      //Curr_El->correct(NodeTable, El_Table, dt, matprops_ptr,
              //                 fluxprops, timeprops_ptr,
	      //                 &elemforceint,&elemforcebed,
	      //                 &elemeroded,&elemdeposited);

	      //made correct() a non-member function so openmp could handle 
	      //this loop so it would be privatized so there wouldn't be a
	      //"race condition."
	      ...
	      double pfheight[6];
	      //polyfitheight( El_Table, Curr_El, coord, pfheight);
  ...
  //printf("step: after corrector loop\n"); fflush(stdout);
  ...
  //printf("step: after calc_wet_dry_orient loop\n"); fflush(stdout);

  /* finished corrector step */

  //  if(statprops_ptr->timereached!=-1.0)
  //    printf("timereached=%g before calc_stats()\n",statprops_ptr->timereached);
  ...
  for(i=0; i<El_Table->get_no_of_buckets(); i++)
	...
	while(currentPtr)
	  {
	    Element* Curr_El=(Element*)(currentPtr->value);
 	    if(Curr_El->get_adapted_flag()>0) //if this is a refined or GOST element don't involve!!!
		Curr_El->calc_shortspeed(1.0/dt);
