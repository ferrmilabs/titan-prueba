...
c      double precision shearX, shearY
...
c      parameter(erosion_rate=0.14)......2.983063e+05.........6.282101e+05.....dec_11_erosion3.
       parameter(erosion_rate=0.025)
c Keith guessed 0.025 because the volume on casitanew2 grew astronomically with 0.15, eroded volume passed 10^8 [m^3]
...
c
     ...
     &           dmax1((1-dtan(slope)/dtan(intfrictang)),0.0d0)**2
...
c         if(IF_STOPPED.eq.2) then
c            write(*,*) "IF_STOPPED=", IF_STOPPED
c         endif
...
c names for clarity
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
         if((DO_EROSION.ne.0).and.(IF_STOPPED.eq.0)) then
