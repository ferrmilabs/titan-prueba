		...
		DISCHARGE* discharge, double d_time)
{
  ...
  /*
  printf("testpointheight = %g %g\n",testpointheight,
	 testpointheight*matprops->HEIGHT_SCALE);
  printf("testpoint x = %g %g\n",testpointx,
	 testpointx*matprops->LENGTH_SCALE);
  printf("testpoint y = %g %g\n",testpointy,
	 testpointy*matprops->LENGTH_SCALE);
  */
  ...
  //fprintf(fp,"calc_stats() 1\n");  fflush(fp);

  /* FILE* fp=fopen("discharge.debug","a");
  ...
  //  printf("myid=%d slope_ave=%g slopevolume=%g\n",myid,slope_ave/slopevolume,slopevolume); fflush(stdout);

  /* printf("(%g,%g) %g %g %g (%g,%g) %g %g %g\n", xC,yC,
	 rC,area,v_ave,vx_ave,vy_ave,g_ave,
	 statvolume,realvolume); */
  i = MPI_Reduce(tempin, tempout, 13, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  /* printf("(%g,%g) %g %g %g (%g,%g) %g %g %g\n",tempout[0],tempout[1],
	 tempout[2],tempout[3],tempout[4],tempout[5],tempout[6],tempout[7],
	 tempout[8]); */
  ...
  if(myid == 0) {
    ...
    //   printf("slopemean=%g slopevolume=%g\n",statprops->slopemean,tempout[9]); fflush(stdout);
    ...
    /* the factor of 3^0.5 is a safety factor, this value was chosen because
       ...
       the line length */
    /*    statprops->piler=sqrt(3.0*(tempout[8]*(matprops->LENGTH_SCALE)*
			       (matprops->LENGTH_SCALE)/tempout[10] -
    ...
    /*  printf("vscale=%g\n",VELOCITY_SCALE);
    printf("(%g,%g) %g %g %g (%g,%g) %g %g %g\n", statprops->xcen,
	   statprops->ycen,statprops->rmean,statprops->area,statprops->vmean,
	   statprops->vxmean, statprops->vymean,statprops->nzmean,
	   -1.0,statprops->statvolume); */
    ...
    //	   statprops->statvolume/statprops->realvolume*100.0,testvolume);
  }
