 ...
 *
 *******************************************************************
 * $Id: tecplot.C 136 2007-06-07 20:18:23Z dkumar $ 
 */
		...
		double v_star)
{
  ...
  if(myid==TARGETPROCA) {printf("at tecplotter 1.0\n"); fflush(stdout);}
  ...
  */
  ...
  char filename[20];
  sprintf(filename,"tecpl%02d%08d.plt",myid,timeprops->iter);
  ...
  fprintf ( fp, "VARIABLES = \"X\", \"Y\", \"PILE_HEIGHT\", \"X_MOMENTUM\", \"Y_MOMENTUM\", \"ELEVATION\" \"PILE_HEIGHT_ELEV\", \"CORRECTSPEED\" \n"); //}
		 ...
		 double v_star)
{
  ...
  //sprintf(filename,"mshplot%08d.plt",timeprops->iter);
