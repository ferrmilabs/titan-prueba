...
{ 
  ...
  if(fp!=NULL) {
    ...
    int intswap;
    char samplefilename[256];
    sprintf(samplefilename,"volumebedsample.%06d",isample);
    fp=fopen(samplefilename,"r");
    if(fp!=NULL) {
      ifpcqvolbed=1;
      double volumescale;
      char stringswap[4096];
      fgets(stringswap,4096,fp);  //Ndeg
      fgets(stringswap,4096,fp);  //Nrand
      fgets(stringswap,4096,fp);  //Nagpow
      fgets(stringswap,4096,fp);  //Nsample
      fscanf(fp,"isample=%d\n",&intswap);
      assert(intswap==isample);
      fgets(stringswap,4096,fp);  //weight
      fgets(stringswap,4096,fp);  //randvar
      fscanf(fp,"volume=%lf\n",&volume);
      fscanf(fp,"bedfrict=%lf\n",&pcqbedfrict);
      printf("PCQ: volume=%g[m^3] bedfrict=%g[deg]\n",volume,pcqbedfrict);
      fflush(stdout);
      fclose(fp);
      double lengthscalefact;
      for(i=0;i<numpiles;i++) {
	lengthscalefact=pow(volume/(pileprops_ptr->pileheight[i]*pileprops_ptr->majorrad[i]*pileprops_ptr->minorrad[i]*PI/2.0),1/3.0); 

	pileprops_ptr->pileheight[i]*=lengthscalefact;
	pileprops_ptr->majorrad[i]*=lengthscalefact;
	pileprops_ptr->minorrad[i]*=lengthscalefact;
	
	printf("Vol=%g [m^3]\n",PI/2.0*pileprops_ptr->pileheight[i]*pileprops_ptr->majorrad[i]*pileprops_ptr->minorrad[i]);
      }
    }
  }
  */
  ...
  //printf("LENGTH_SCALE=%g HEIGHT_SCALE=%g GRAVITY_SCALE=%g doubleswap=%g MAX_NEGLIGIBLE_HEIGHT=%g smallest_axis=%g number_cells_across_axis=%d\n",matprops_ptr->LENGTH_SCALE,matprops_ptr->HEIGHT_SCALE,matprops_ptr->GRAVITY_SCALE,doubleswap,matprops_ptr->MAX_NEGLIGIBLE_HEIGHT,matprops_ptr->smallest_axis,matprops_ptr->number_of_cells_across_axis);
